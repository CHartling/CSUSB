...
class SearchAlgorithms:
    ...
    def depthFirstSearch(self, start, goal, graph):
        ...
        while fringe:
            ...
            if node not in visited:
                visited.add(node)
                successors = reversed(graph[node])
                for neighbor in successors:
                    fringe.append((neighbor, path + [neighbor]))
            ...
            if node == goal:
                ...
                for neighbor in successors:
                    if neighbor not in visited:
                        ...
                        Ncost = cost + weights[(node, neighbor)]
                        queue.put((Ncost, neighbor, path + [neighbor]))
    ...
    def AStar(self, start, goal, graph, weights, heuristic):
        ...
        visited.add(start)
        while not queue.empty():
            (ghCost, cost, node, path) = queue.get()
            if node == goal:
                return {"Returned solution: ": path, "Expanded cities: ": visited}
                ...
                for neighbor in successors:
                    if neighbor not in visited:
                        visited.add(neighbor)
                        Ncost = cost + weights[(node, neighbor)]
                        cCost = Ncost + heuristic[neighbor]
                        queue.put((cCost, Ncost, neighbor, path + [neighbor]))
